...
const SuperAdminSidebar: React.FC<SuperAdminSidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className={cn(
      "fixed inset-y-0 z-50 w-72 bg-gradient-to-b from-slate-900 to-slate-800 text-white flex flex-col",
      isRTL ? "right-0 border-l border-slate-700" : "left-0 border-r border-slate-700"
    )}>
      {/* Header */}
      <div className={cn(
        "flex h-16 items-center justify-between px-4 border-b border-slate-700",
      ...
      <div className="px-4 py-4 border-b border-slate-700">
        ...
        <Badge variant="secondary" className={cn(
          "mt-2 bg-green-500/20 text-green-400 border-green-500/30 flex items-center",
          isRTL ? "flex-row-reverse" : ""
        )}>
          <Shield className={cn(
            "h-3 w-3",
      ...
      <div className="border-t border-slate-700 p-4">
        <Button
          ...
          variant="ghost"
          className={cn(
            "w-full text-slate-300 hover:bg-red-600/20 hover:text-red-400 flex items-center",
