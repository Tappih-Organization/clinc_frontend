        ...
        <div dir={isRTL ? "rtl" : "ltr"}>
          ...
          <Tabs value={selectedTab} onValueChange={setSelectedTab} className="w-full">
            ...
            <TabsContent value="invoices" className="space-y-3 xs:space-y-4">
              <Card>
                <CardHeader className={cn("pb-3 w-full", isRTL && "text-right")}>
                  <CardTitle
                    className={cn("text-base xs:text-lg sm:text-xl w-full block", isRTL && "text-right")}
                    style={{ textAlign: isRTL ? 'right' : 'left' }}
                  >
                    {t('Invoice Management')}
                  </CardTitle>
                  <CardDescription
                    className={cn("text-xs xs:text-sm w-full block", isRTL && "text-right")}
                    style={{ textAlign: isRTL ? 'right' : 'left' }}
                  >
                ...
                <CardContent className="px-3 xs:px-4 sm:px-6">
                  ...
                  <div className="hidden lg:block">
                    <Table dir={isRTL ? "rtl" : "ltr"}>
                      <TableHeader>
                        <TableRow>
                          <TableHead className={cn(isRTL && "text-right")}>{t('Invoice')}</TableHead>
                        ...
                        {loading ? (
                          ...
                          filteredInvoices.map((invoice) => (
                            <TableRow key={invoice._id} className="hover:bg-gray-50">
                              ...
                              <TableCell className={cn(isRTL && "text-right")}>
                                <Badge variant="secondary" className={cn("text-xs", getStatusColor(invoice.status), isRTL && "flex-row-reverse")}>
                                  {getStatusIcon(invoice.status)}
                                  <span className={cn("capitalize", isRTL ? "mr-1" : "ml-1")}>{invoice.status}</span>
                                </Badge>
                              </TableCell>
                              <TableCell className={cn(isRTL && "text-right")}>
                ...
                <CardContent className="px-3 xs:px-4 sm:px-6">
                  ...
                  <div className="hidden lg:block">
                    <Table dir={isRTL ? "rtl" : "ltr"}>
                      <TableHeader>
                        <TableRow>
                          <TableHead className={cn(isRTL && "text-right")}>{t('Transaction')}</TableHead>
                          <TableHead className={cn(isRTL && "text-right")}>{t('Patient')}</TableHead>
                          <TableHead className={cn(isRTL && "text-right")}>{t('Invoice')}</TableHead>
                          <TableHead className={cn(isRTL && "text-right")}>{t('Amount')}</TableHead>
                          <TableHead className={cn(isRTL && "text-right")}>{t('Method')}</TableHead>
                          <TableHead className={cn(isRTL && "text-right")}>{t('Status')}</TableHead>
                          <TableHead className={cn("text-right", isRTL && "text-left")}>{t('Actions')}</TableHead>
                        </TableRow>
                      </TableHeader>
                        ...
                        {loading ? (
                          ...
                          filteredPayments.map((payment) => (
                            <TableRow key={payment._id} className="hover:bg-gray-50">
                              ...
                              <TableCell className={cn(isRTL && "text-right")}>
                                <Badge variant="secondary" className={cn("text-xs", getStatusColor(payment.status), isRTL && "flex-row-reverse")}>
                                  {getStatusIcon(payment.status)}
                                  <span className={cn("capitalize", isRTL ? "mr-1" : "ml-1")}>{payment.status}</span>
                                </Badge>
                              </TableCell>
                              <TableCell className={cn("text-right", isRTL && "text-left")}>
                                <DropdownMenu>
